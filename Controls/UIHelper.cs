    ...
    internal static class UIHelper
    {
        ...
        public static async Task<Tuple<int, int>> GetImageInfoForRendering(string imageFilePath)
        {
            try
            {
                ...
                file = await folder.GetFileAsync(imageFilePath);
                using (IRandomAccessStream fileStream =  await file.OpenAsync(FileAccessMode.Read))
                {

                    BitmapDecoder dec = await BitmapDecoder.CreateAsync(BitmapDecoder.JpegDecoderId, fileStream);
                 
                    BitmapFrame frame = await dec.GetFrameAsync(0);

                    // Store image width and height for following rendering
                    return new Tuple<int, int>(checked((int)frame.PixelWidth), checked((int) frame.PixelHeight));
                }
            }
            catch (Exception ex)
            {
